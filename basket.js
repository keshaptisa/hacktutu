...
function renderCard(item) {
  ...
  card.innerHTML = `
    ...
    <p>Переход ведет на реальную страницу оформления на Туту.</p>
...
async function loadBasket() {
  ...
  try {
    ...
    basketCopy.textContent =
      data.items.length > 0
        ? `Нашли ${data.items.length} реальных ссылок из Tutu MCP. Итоговый бюджет сценария: ${formatBudget(data.total_price_rub)}.`
        : "Сценарий собран, но Tutu MCP не вернул живых ссылок на оформление по этому маршруту.";
